...
export function BargeProvider({ children }: { children: ReactNode }) {
  ...
  const { data, isLoading } = useQuery<Barge[]>({
    ...
    queryFn: async (): Promise<Barge[]> => {
      const response = await axios.get(`${process.env.API_ENDPOINT}/${process.env.API_VERSION}/barges`);
      return response.data as Barge[];
    },
  });
  ...
  const getById = async (id: string) => {
    ...
    setSelectedTugboat(res.data as Barge);
  }
