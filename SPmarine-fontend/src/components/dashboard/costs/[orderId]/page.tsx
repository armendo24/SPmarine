...
export default function CostDetailPage(): React.ReactElement {
  const params = useParams();
  const router = useRouter();
  const tugboatId = params.tugboatId as string;
  const orderId = params.orderId as string;
  
  const { getByIds, isLoading } = useCost() as { getByIds: (tugboatId: string, orderId: string) => Cost | null, isLoading: boolean };
  const { getById: getTugboat } = useTugboat() as { getById: (id: string) => Tugboat | null };
