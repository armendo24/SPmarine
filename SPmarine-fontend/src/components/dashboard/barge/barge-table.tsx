...
export function BargeTable({ count, rows, page, rowsPerPage = 0, onPageChange, onRowsPerPageChange }: BargeTableProps): JSX.Element {
                ...
                <TableRow hover key={row.id} selected={isSelected}>
                  ...
                  <TableCell>
                    <Box component={RouterLink} href={`barges/${row.id}`}>
                      <Link><Typography variant="subtitle2">{row.name}</Typography></Link>
                    </Box>
                  </TableCell>
